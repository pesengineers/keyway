...
1. **Deploy the logging fix** (`602ec87`, CI green): update the `keyway` container in the Docker tab **between jobs** (check `nvidia-smi` shows ~60 MiB and no `queue-*` directory is newer than a minute). Restarting mid-job leaves a row in `processing`; the re-read guard will not rescue it, it needs a reset via the form.
2. **Watch the first day.** After ~12 h (most of the backlog), review `done` titles/synopses and every `flagged_*`/`error`/`unprocessable` row per `docs/operations.md`.
