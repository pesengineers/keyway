...
1. **Cycle Active off/on on Process Queue in the n8n UI** (https://n8n.pesengineers.dev/workflow/mp5gviKuHu9iIfPo). The 5-minute schedule is saved as version `15ef01d2` but n8n's scheduler is not armed after the API unpublish/publish (`workflow_entity.activeVersionId` still `33171b76`, log "Deregistered all crons"). After toggling, confirm a run appears within 5 minutes.
