...
Keyway is deployed and working on the production Unraid host `pes-dev` with a rotated Graph secret. The analysis step now runs through **OpenRouter / `openai/gpt-4o-mini`** after the local Ollama models misclassified the first real queue item (ADR 006); Ollama remains as an offline fallback. Model-supplied dates are now discarded unless backed by verbatim transcript evidence (commit `124e067`; deployed and verified on the host). Real SharePoint videos have been pulled through Microsoft Graph, transcribed on the Quadro P2000, analyzed by a local Ollama model, and returned as structured JSON. All six original GitHub issues are closed. The n8n workflow that feeds Keyway from the existing 281-item queue exists (`Keyway - Process Queue`, inactive, source in `n8n/`). **The pipeline is in production.** All three workflows are active. The first end-to-end production-path run (execution 2507, row 1) completed in 57 s: `safe`, title and synopsis written to the SharePoint item, filename date used, model-guessed date correctly discarded. The queue (281 videos) drains at one per 15-minute cycle, about 3 days; shorten the interval once a day of runs looks clean.
