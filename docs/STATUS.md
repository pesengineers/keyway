...
1. **Re-paste the rotated Graph secret.** Root cause found: the template on flash still held the ORIGINAL (leaked, now deleted) secret; the 22:24 template edit for the OpenRouter variables reloaded it and reverted the rotation. Docker > keyway > Edit > `GRAPH_CLIENT_SECRET` > paste the rotated value from 1Password (must not start with `uNY`) > Apply. If the rotated value was not saved, run `New-KeywayGraphApp.ps1 -RotateSecret` first. Verify with the token test in runbook 1.7b.
