...
Last updated 2026-09-06 (late; helper workflows live, Graph secret blocked). This is the single page to read before doing anything. It states the exact state of every system Keyway touches, what is done, what is next, and what is unresolved. The chronological record is `docs/deployment-journal.md`; this page is the snapshot.

## One-paragraph summary

Keyway is deployed and working on the production Unraid host `pes-dev` with a rotated Graph secret. The analysis step now runs through **OpenRouter / `openai/gpt-4o-mini`** after the local Ollama models misclassified the first real queue item (ADR 006); Ollama remains as an offline fallback. Model-supplied dates are now discarded unless backed by verbatim transcript evidence (commit `124e067`; deployed and verified on the host). Real SharePoint videos have been pulled through Microsoft Graph, transcribed on the Quadro P2000, analyzed by a local Ollama model, and returned as structured JSON. All six original GitHub issues are closed. The n8n workflow that feeds Keyway from the existing 281-item queue exists (`Keyway - Process Queue`, inactive, source in `n8n/`). Helper workflows `Keyway - Seed Queue` (daily discovery, de-duplicated) and `Keyway - Reset Queue Rows` (login-protected form) are **active**. `Keyway - Process Queue` is built, annotated, and verified up to the Keyway call, but is **not active**: the last two manual runs failed at Keyway's Graph download with `AADSTS7000215 Invalid client secret`, which only the Entra admin can fix. Once the secret is valid, one manual run plus activation finishes go-live.
...
- **`Keyway - Process Queue`** (`mp5gviKuHu9iIfPo`), **inactive**. Three manual runs so far (2501: wrong verdict from local model, since fixed; 2500 and 2503: 502 from Keyway because of the Graph secret). Canvas has section notes; Keyway HTTP node continues on connection errors so rows never get stuck in `processing`; error workflow set to `Send Error to Sentry`.
...
1. **Fix the Graph client secret** (Entra admin). Container value = template value, 40 chars, but Entra returns `AADSTS7000215 Invalid client secret`. It authenticated fine at ~16:14 the same day; the template was re-saved at 22:24 and/or the old secret deletion removed the wrong one. Check App registrations > Keyway Video Worker > Certificates & secrets; either re-run `New-KeywayGraphApp.ps1 -RotateSecret` and update `GRAPH_CLIENT_SECRET`, or re-paste the current value from 1Password. Verify with the token test in runbook 1.7b.
2. **Test and activate the Process Queue workflow.** Rows 1 and 2 are already reset to `pending`. Execute once (MCP `execute_workflow` or the n8n Execute button), confirm row 1 becomes `done` and the SharePoint item shows Title/Synopsis, then `publish_workflow`. Open https://n8n.pesengineers.dev/workflow/mp5gviKuHu9iIfPo, confirm the SharePoint node shows credential `Sharepoint video process`, execute once manually, check that the oldest pending row becomes `done` (or `flagged_*`/`unprocessable`) and that the SharePoint item's Title/Synopsis were written. Then activate. Checklist in `docs/n8n-integration.md` §6. Consider a new seed workflow later that pages Graph and skips items already queued. The frozen workflows stay untouched.
