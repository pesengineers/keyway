...
23e. **MCP `unpublish_workflow` + `publish_workflow` can leave a scheduled workflow with no cron registered** while the API still reports `active: true` and a new `activeVersionId`. n8n's database keeps the old version and logs only "Deregistered all crons". Symptom: schedule silently stops. Fix: toggle Active off/on in the n8n UI. Verify a schedule change by waiting one interval and checking executions (or `docker logs n8n | grep -i cron`), never by the API response. Prefer changing schedule parameters in the UI, or at least confirm the first run after any API republish.
