...
6. **For scheduled workflows, confirm with `search_workflow_executions` after one interval.** Do not use `docker logs n8n` (it only prints "Deregistered all crons" on every publish) or a copied SQLite file (it lags the live WAL by many minutes) to judge whether the schedule is armed; both produced a false alarm on 2026-09-07.
