...
### 2026-09-07 (go-live)
- Throughput: on request, changed Process Queue to every 5 min and 2 rows per run via `update_workflow` (`renameNode` needs `oldName`, not `nodeName`). Republished. Expected ~24 videos/h; backlog ~12 h.
