...
### 2026-09-07 (go-live)
- Throughput: on request, changed Process Queue to every 5 min and 2 rows per run via `update_workflow` (`renameNode` needs `oldName`, not `nodeName`), then unpublish/publish. **Defect found:** no run occurred for 20+ minutes; n8n log shows "Deregistered all crons" and nothing else; `workflow_entity.activeVersionId` still the 15-minute version although the API returns the new one and `active: true`. REST API is not reachable with the MCP token (401), so the fix requires the UI Active toggle. Row 3 (`Designing Wood Structures...`, `safe`, 164 s) was the last scheduled run at 00:30 UTC.
